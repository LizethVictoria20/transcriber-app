...
export default function TranscribeView({
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      // Save to Supabase Database and Storage
      const newItem = await dbService.createTranscription(
        {
      ...
      onTranscriptionComplete(newItem);
    } catch (err: any) {
      console.error("Error during transcription:", err);
      const errorMessage = err.message || "Error desconocido";
      setError(`Ocurrió un error: ${errorMessage}`);

      // Optionally log failure to DB if desired, skipping for now to keep it simple
    } finally {
