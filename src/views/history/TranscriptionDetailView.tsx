    ...
    <div className="max-w-6xl mx-auto py-8 px-4 font-sans text-slate-800">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 flex items-center gap-2 mt-1">
              <span className="bg-slate-100 px-2 py-0.5 rounded text-xs font-medium border border-slate-200 uppercase tracking-wide">
        ...
        <div className="flex gap-2">
          {item.error && (
            <span className="bg-red-50 text-red-600 px-3 py-1.5 rounded-lg text-sm font-bold border border-red-100 flex items-center gap-2">
              <HiOutlineExclamationCircle className="w-5 h-5" />
              Fallido
            </span>
          )}
          {!item.error && (
            <span className="bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-lg text-sm font-bold border border-emerald-100 flex items-center gap-2">
        ...
        >
          ...
          <div
            className={`text-xl font-bold ${
              item.error ? "text-red-700" : "text-blue-700"
            }`}
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 mb-8 shadow-sm">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-purple-50 text-purple-700 rounded-lg text-sm font-bold cursor-pointer hover:bg-purple-100 transition-colors disabled:opacity-50"
          >
            {isGeneratingTags ? (
              <>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {tags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1.5 px-3 py-1 bg-slate-100 text-slate-700 text-sm font-medium rounded-full border border-slate-200 group"
            >
              {tag}
              <button
                onClick={() => removeTag(tag)}
                className="w-4 h-4 rounded-full flex items-center justify-center text-slate-400 hover:bg-red-100 hover:text-red-500 transition-colors"
              >
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              placeholder="+ Añadir..."
              className="w-32 px-3 py-1 text-sm bg-transparent border border-dashed border-slate-300 rounded-full focus:w-48 focus:border-blue-500 focus:ring-0 transition-all outline-none placeholder:text-slate-400"
            />
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              {suggestedTags.map((tag) => (
                <button
                  key={tag}
                  onClick={() => addTag(tag)}
                  className="text-xs bg-white text-purple-600 px-3 py-1 rounded-lg border border-purple-100 hover:border-purple-300 hover:shadow-sm transition-all flex items-center gap-1"
                >
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-8">
        <div className="flex-1 bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              className="flex-1 min-w-[140px] flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-50 text-slate-700 rounded-xl border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all text-sm font-semibold cursor-pointer disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex-1 min-w-[140px] flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-50 text-slate-700 cursor-pointer rounded-xl border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all text-sm font-semibold disabled:opacity-50"
            >
            ...
            <button
              ...
              className="flex-1 min-w-[140px] flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-50 text-blue-700 cursor-pointer rounded-xl border border-blue-200 hover:bg-blue-100 transition-all text-sm font-semibold disabled:opacity-50"
            >
              {isGeneratingPdf ? (
                <span className="loader w-4 h-4" />
              ) : (
        ...
        {!item.error && (
          <div className="flex-1 bg-linear-to-br from-blue-50 to-indigo-50 p-5 rounded-2xl border border-blue-100 shadow-sm">
            <h4 className="text-xs font-bold text-blue-400 uppercase tracking-wider mb-4 flex items-center gap-1">
              <HiSparkles className="w-3 h-3" />
              Análisis Avanzado
            </h4>
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className="flex-1 min-w-40 flex items-center justify-center gap-2 px-4 py-2.5 bg-white text-blue-700 rounded-xl cursor-pointer border border-blue-200 shadow-sm hover:shadow hover:-translate-y-0.5 transition-all text-sm font-bold disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none"
              >
                {generatingTable ? (
              ...
              <button
                ...
                className="flex-1 min-w-40 flex items-center justify-center gap-2 px-4 py-2.5 bg-white text-indigo-700 rounded-xl cursor-pointer border border-indigo-200 shadow-sm hover:shadow hover:-translate-y-0.5 transition-all text-sm font-bold disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none"
              >
                {generatingPageTable ? (
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 overflow-hidden min-h-[600px]">
        {viewMode === "editor" && (
          <div className="flex flex-col h-full">
            <div className="border-b border-slate-100 p-3 flex items-center gap-2 bg-slate-50/50 flex-wrap">
              {isEditing ? (
                <>
                  <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
                    <button
                      className="p-2 hover:bg-slate-50 rounded text-slate-700 font-bold cursor-pointer"
                      onClick={() => execCmd("bold")}
                    >
                      B
                    </button>
                    <button
                      className="p-2 hover:bg-slate-50 rounded text-slate-700 italic cursor-pointer"
                      onClick={() => execCmd("italic")}
                    >
                      I
                    </button>
                    <button
                      className="p-2 hover:bg-slate-50 rounded text-slate-700 underline cursor-pointer"
                  ...
                  <div className="flex bg-white rounded-lg border border-slate-200 p-1 shadow-sm">
                    <button
                      className="px-3 py-1.5 hover:bg-slate-50 rounded text-xs font-bold text-slate-600 cursor-pointer"
                ...
                <span className="text-xs font-bold text-slate-400 uppercase tracking-wide px-3">
                  Modo Lectura
                </span>
              )}
              ...
              <div className="ml-auto flex gap-2">
                {!isEditing ? (
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-700 rounded-lg text-xs font-bold hover:bg-blue-100 transition-colors cursor-pointer"
                  >
                  ...
                  <>
                    <button
                      onClick={toggleEditMode}
                      className="px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                    >
                      Cancelar
                    </button>
                    <button
            ...
            <div
              className={`p-8 md:p-12 outline-none min-h-[500px] prose prose-slate max-w-none ${
                !isEditing ? "bg-slate-50/30" : "bg-white"
              }`}
